...
class Server(QtWidgets.QWidget):
    ...
    def handle_group_notification(self, group_name, member_name, client_socket):
        ...
        if group_name not in self.groups:
            self.groups[group_name] = {"members": set(), "sockets": set()}
    
        self.groups[group_name]["members"].add(member_name)
        self.groups[group_name]["sockets"].add(client_socket)
    
        # Notify all group members about the new member
        for socketed in self.groups[group_name]["sockets"]:
            try:
                socketed.sendall(
    ...
    def broadcast_group_message(self, group_name, sender_name, message):
        ...
        if group_name in self.groups:
            for client_socket in self.groups[group_name]["sockets"]:
                try:
                    client_socket.sendall(
                        f"&{group_name}:{sender_name}: {message}".encode()
                    )
                except OSError:
